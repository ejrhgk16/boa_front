...
const  Page = (props : any) => {
  ...
  const targetUrl = '/api/store/etc/script'

   const [scriptList, setScriptList] = useState<any[]>([]); // 초기 전체 데이터
    ...
    const addClientType = () => {
      setScriptList([...scriptList, { num: scriptList.length, script_name: '', script_content:''}]);
    };

    const removeClientType = (id:any) =>{
      setScriptList(scriptList.filter(item => item.id !== id));
    }


    const onChangeValue = (item : any, name : any, value:any)=>{
      const updatedList = scriptList?.map((e)=> {return e.num === item.num ? {...item, [name] : value} : e} )

      setScriptList(updatedList)
    }
    ...
    const sendData = async ()=>{
      ...
      const targetUrl = "/api/store/etc/script/update"

      const param = {store_code : store_code, list : scriptList}
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      ...
      <div className="">
        ...
        {scriptList?.map((item, index)=>(
            <div key={index}>
            <input
              id={"script_name"+index}
              type="text"
              defaultValue={item.script_name}
              placeholder="명칭 입력"
              ...
              onChange={(e)=>{onChangeValue(item, 'script_name', e.target.value)}}
            />

          <textarea
            id={"script_content"+index}
            rows={6}
            defaultValue={item.script_content}
            placeholder="내용 입력"
            className="w-full border-[1.5px] border-stroke bg-transparent px-5 py-3 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
            onChange={(e)=>{onChangeValue(item, 'script_content', e.target.value)}}
          ></textarea>
