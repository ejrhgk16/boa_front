...
const Page = () => {
  ...
  return (
    ...
    <button type="button" className="py-2.5 px-5 me-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700" onClick={()=>{window.open(process.env.NEXT_PUBLIC_FRONT_DOMAIN+"/boa/etc/role/add", '상세페이지', 'width=550,height=700');}}>추가</button>
              ...
              <tr key={key} className="text-center">
                ...
                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark text-center" >
                  <h5 className="text-black dark:text-white font-medium">
                    <Link href={process.env.NEXT_PUBLIC_FRONT_DOMAIN+"/boa/etc/role/detail?role_id="+item.role_id} rel="noopener noreferrer" target="_blank" className="hover:underline" onClick={(e)=>{
                      e.preventDefault();
