...
const  ClientTypePage = () => {
  ...
  const targetUrl = '/api/admin/client/type/list'

   const [clientTypeList, setClientTypeList] = useState<any[]>([]); // 초기 전체 데이터
  ...
  // const [searchTerm, setSearchTerm] = useState(''); // 검색어 상태 관리
    ...
    const addClientType = () => {
      setClientTypeList([...clientTypeList, { id: clientTypeList.length + 1, name: '', memo:'', status:'Y' }]);
    };

    const removeClientType = (id:any) =>{
      setClientTypeList(clientTypeList.filter(item => item.id !== id));
    }


    const changeClientTypeInfo = (item : any)=>{
      const updatedList = clientTypeList?.map((e)=> e.id === item.id ? item : e )
    ...
    const sendData = async ()=>{
      ...
      const targetUrl = "/api/admin/client/type/update"

      try {
        const res  = await fetchToFrontServer.boaPost(targetUrl, clientTypeList)
  ...
  return (
    ...
    <DefaultLayout>
    <Breadcrumb pageName="클라이언트 종류 관리" />
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      ...
      <div className="">
        ...
        {clientTypeList?.map(item=>(
          <SwitcherTwo element={item} onChangeInfo={changeClientTypeInfo} key={item.id}></SwitcherTwo>
