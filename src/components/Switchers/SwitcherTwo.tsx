...
const SwitcherTwo: React.FC<{element: any, onChangeInfo:any, columName1?:any, columName2?:any, inputType?:any}> = ({element, onChangeInfo, columName1='이름', columName2='설명', inputType='text'}) => {

  const [enabled, setEnabled] = useState(element.status ==='Y' ? true:false);
  ...
  return (
    <div x-data="{ switcherToggle: false }" style={{
      display: 'flex',
      ...
      >
        ...
        <div className="relative">
          <input type={inputType} placeholder="Default Input" className=" rounded-lg border-[2.0px] border-stroke bg-transparent px-5 py-3 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary" name="name" value={element.name !== null ? element.name : ''} onChange={onChange}/>
        </div>
      ...
      >{columName2} &nbsp;&nbsp;
        <div className="relative">
        <input type={inputType} placeholder="Default Input" className="w-150 rounded-lg border-[2.0px] border-stroke bg-transparent px-5 py-3 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary" name="memo" value={element.memo !== null ? element.memo : ''} onChange={onChange}/>
        </div>
        ...
        <div className="relative">
          ...
          <div
            className={`dot absolute -top-1 left-0 h-7 w-7 rounded-full bg-white shadow-switch-1 transition ${
              enabled//element.status === 'Y'?true:false
